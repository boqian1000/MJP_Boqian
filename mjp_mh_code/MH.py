...
def MHUpdate(observation, path_old, k, lambs, alpha, beta, priors, cov):
    ...
    c1, d1 = priors[2]
    c2, d2 = priors[3]
#    var_a, var_b, var_l1, var_l2 = vars
...
def MHsampler(observation, pi_0, sample_n, T_interval, k, priors, cov):
    ...
    path_old = MJPpath(t_start=T_interval[0], t_end=T_interval[1], rate_matrix=rate_matrix, initial_pi=pi_0)
...
#        print alpha_old, beta_old, lambs_old
    return alpha_list, beta_list, lamb1_list, lamb2_list

def MH(pi_0, sample_n, T_interval, k, priors, var):
    alpha_list = []
    beta_list = []
    lamb1_list = []
    lamb2_list = []
    ...
    beta_old = 1.5
    lambs_old = [1., 1.]
    observation = Observation(O = [1] * 10, T = range(1, 11), t_start= 0.0, t_end = 11.0)
    rate_matrix = constructor_rate_matrix(alpha_old, beta_old)
    path_old = MJPpath(t_start=T_interval[0], t_end=T_interval[1], rate_matrix=rate_matrix, initial_pi=pi_0)
    path_old.generate_newpath()
    path_list.append(copy.deepcopy(path_old))
    for i in range(sample_n):
        my_print(i, 1000)
        path_new, alpha_new, beta_new, lambs_new = MHUpdate(observation, path_old, k, lambs_old, alpha_old, beta_old, priors, var)
        path_list.append(copy.deepcopy(path_new))
        alpha_list.append(alpha_new)
        beta_list.append(beta_new)
        lamb1_list.append(lambs_new[0])
        lamb2_list.append(lambs_new[1])
        path_old, alpha_old, beta_old, lambs_old = path_new, alpha_new, beta_new, lambs_new
        observation = SampleMMPP(path_old, lambs_old)
    return alpha_list, beta_list, lamb1_list, lamb2_list
