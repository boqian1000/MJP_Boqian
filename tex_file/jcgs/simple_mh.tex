...
\end{align*}
 ...
 %   Sequentially update $\fwd^\theta_i(\cdot)$ at time $w_i \in W$ as: 
    ...
    \State \textbf{Propose $\vartheta \sim q(\cdot| \theta)$.}
      ...
      \State \textbf{Accept/reject:} With probability $\texttt{acc}$, set the new parameter $\theta'$ to $\vartheta$, else set it to $\theta$: %Here 
...
We however note that even with state values $V$ marginalized out, $\theta$ is updated {\em conditioned on $W$}. 
