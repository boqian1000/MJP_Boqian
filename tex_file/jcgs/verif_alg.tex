...
This MH step has stationary distribution over $(\theta',\vartheta')$ proportional to
$P(\theta')q(\vartheta'|\theta)$ $\text{PoissProc}(W|\Omega(\theta',\vartheta'))
P(X|W,\theta',\vartheta')$, so that the triplet $(\theta', \vartheta',W)$ has the same distribution as $(\theta, \vartheta,W)$.
...
Another approach to parameter inference that integrates out state values follows~\citet{FearnSher2006}. 
 ...
 Unlike with $W$, one cannot a priori bound the number of transitions between two successive observations, so that~\citet{FearnSher2006} have to use matrix exponentials of $A$ (rather than $B$) to calculate transition probabilities.
 ...
 This algorithm, described in section~\ref{sec:pmcmc} in the appendix, uses particle filtering to get an unbiased estimate of $P(X|\theta)$. 
...
Uniformization forms such a hierarchical representation for MJPs: first sample $\theta$, then sample the latent $W$, and use this to sample MJP state values. 
...
This recasts our contribution as a rewriting of uniformization that now includes the auxiliary parameter $\vartheta$. Our swap operator forms a particular Markov kernel that exploits this reparametrization for fast mixing. 
