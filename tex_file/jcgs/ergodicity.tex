...
\begin{assumption}
For any positive $\epsilon > 0$ and $K_1 > 1$, there exists $\theta_3$, such that for $\theta > \theta_2$, $P(\frac{1}{K_1}\frac{\Omega(\theta')}{\Omega(\theta)} \leq K_1 | \theta) \geq 1 - \epsilon$, where $\theta' \sim q(\cdot | \theta)$.
\end{assumption}
...
\end{proposition}
\begin{proof}
From previous lemma, for any $\epsilon > 0$ and a determined $K_0 > 1$, given $\frac{1}{K_0} \Omega(\theta) \leq \Omega(\theta') \leq K_0 \Omega(\theta)$, $\exists \theta_0 > 0, W_0 > 0$, such that for any $|W| > W_0$ and $\theta > \theta_0$, we have $|\frac{P(X | W, \theta' , \theta)}{P(X | W, \theta , \theta')} - \frac{P(X | \theta')}{P(X | \theta)}| < \epsilon / 2$. 
\begin{align*}
...
Given the current state $(W, \theta)$, from assumption 7, there exists $\theta_2 > 0$, such that for $\theta > \theta_2$, $P(\frac{q(\theta | \theta')p(\theta')}{q(\theta' | \theta)p(\theta)} \leq 2 | \theta) > 1 - \epsilon / 3$.\\
Consider the $K_0 $ in previous paragraph. Given the current state $(W, \theta)$, from assumption 8, there exists $\theta_3 > 0$, such that for $\theta > \theta_3$, $P(\frac{1}{K_0} \frac{\Omega(\theta')}{\Omega(\theta)} \leq K_0 | \theta) > 1 - \epsilon / 3 $.\\
So given the current state $(W, \theta)$, and $K_0$, $P(\{ \frac{1}{K_0}\leq \frac{\Omega(\theta')}{\Omega(\theta)} \leq K_0 \} \cap \{ \frac{q(\theta | \theta')p(\theta')}{q(\theta' | \theta)p(\theta)} \leq 2\}| \theta ) > 1 - 2\epsilon / 3 $, for any $\theta > \max(\theta_2, \theta_3)$.\\
For $\epsilon > 0$, there exists $\theta_4 > 0$ such that for $\theta > \theta_4$ we have $P(|W| > W_0 | W, \theta, \theta') > 1- \epsilon / 2$.
From previous lemma, we have $P(|\frac{P(X | W, \theta' , \theta)}{P(X | W, \theta , \theta')} - \frac{P(X | \theta')}{P(X | \theta)}| < \epsilon / 2 | W, \theta, \theta') > 1- \epsilon / 2$
