...
$\prod P(x_o | s_o, \theta) \in [\eta_1, \xi_1]$ for any state $s_o$ and $\theta$.%\vinayak{Is this for all $s_o$?}
  \label{asmp:obs_bnd}
\end{assumption}
...
Then, for any $\epsilon$, there exists $\theta_\epsilon$ as well as a set a set $\E_\epsilon \subseteq \{(W', \theta'): |\alpha_I(\theta,\theta',X) - \alpha(\theta,\theta',W',X)| \le \epsilon\}$, such that for $\theta \ge \theta_\epsilon$ and any $\theta'$, we have
...
\end{align*}
By corollary \ref{corol:integ_bound}, there exists $\eta_1 , \theta_1 > 0$ such that for $\theta \in B_{0, \theta_1}^c$, we have 
\begin{align*}
\int \Omega(\nu) q(\nu | \theta)d\nu \leq \eta_1 \Omega(\theta).
\end{align*}
So for $\theta \in B_{0, \theta_1}^c$, we have 
\begin{align*}
...
For $\tilde{I}_{1,\E_\epsilon}$, by corollary \ref{corol:integ_bound}, and bounding the acceptance probability by $\alpha_I + \epsilon$ to get, for $\theta \in B_{0, \theta_1}^c$
\begin{align*}
...
\end{align*}
For $\tilde{I}_{1,\E^c_\epsilon}$, from assumption \ref{asmp:integ_bound}, there exists $\eta_0 , \theta_0 > 0$ such that for $\theta \in B_{0, \theta_3}^c$, we have 
\begin{align*}
\int_\Theta \Omega(\nu)^2 q(\nu | \theta)d\nu \leq \eta_0 \Omega(\theta)^2.
\end{align*}
...
\end{align*}
Then for $\theta \in B_{0,\max(\theta_0, \theta_1, \theta_\epsilon)}$, we have the following. 
\begin{align*}
  \int \Omega(\theta') P(d\theta'| W, \theta, \vartheta)  
...
\end{align*}
For $\theta \in B_{0,\max(\theta_0, \theta_1, \theta_\epsilon)}$, we have %\boqian{need to change afterwards}
\begin{align*}
