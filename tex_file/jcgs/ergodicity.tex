...
To understand this, recall $B(\theta, \theta') = I+\frac{A(\theta)}{\Omega(\theta, \theta')}$
...
\begin{assumption}
For any $\epsilon>0$,  there exist $M_\epsilon$, $\theta_{4,\epsilon}$
such that for $\theta$ satisfying $\| \theta \|  > \theta_{4,\epsilon}$,
...
\begin{assumption}
For any $\epsilon > 0$ and  $K > 1$, there exists $\theta_{5,\epsilon}^K$ such that 
for $\theta$ satisfying $\| \theta \|  > \theta_{5,\epsilon}^K$, the 
condition
...
\phi_1(W{''}, \theta'',\vartheta'') $ for some probability measure $\phi_1$ 
independent of the initial state.
\label{lem:small_set}
\end{lemma}
\begin{proof} Recall the definition of an $n$-small set from 
...
\begin{align*}
 ...
 \sum_{S,T} P(W'| S, T, \theta', \theta)P(S,T | W, \theta, \vartheta, X)
\alpha(\theta', \theta, W';X)
\end{align*}
...
\end{align*}
Here $\delta_1$ is the lower bound from Proposition~\ref{prop:self_tr}.
With $S(t)$ fixed at $s_0$, $W'$ is a Poisson process with rate
%$r(\theta', \theta, s_0) = 
$\Omega(\theta') + \Omega(\theta) - A_{s_0}(\theta)$.
%$> \epsilon_1 > 0$.
...
\qquad \text{(since $\Omega(\theta) \le M$)}.
...
\label{eq:acc}
\end{align*}
Using assumption \ref{asmp:ideal_geom} to bound $\alpha_I(\theta',\theta;X)$,
and from equations~\eqref{eq:marg} and~\eqref{eq:acc}, 
we then have
\begin{align*}
...
We first establish a proposition needed when $\Mx{\theta}$ is unbounded 
as $\theta$ increases.
...
$\Omega(\theta')$ are sufficiently close.
  ...
  $ for $K_01$ satisfying $(1 + \frac{1}{K_0})k_1 \ge 2$. Write $\mW$ for the
...
Let $P_{st}(V_X|\theta)$ be the \boqian{stationary} probability of the same vector, sampled
...
\numberthis
\end{align*}
From assumption \ref{asmp:mono_tail}, there exists $\theta_0$, such that $\Omega(\theta)$ increases as $\| \theta \|$ increases, for $\theta$ satisfying $\| \theta \| > \theta_0$.
Given current state $(W, \theta)$, set $\tilde{\theta}_\epsilon =\max(\theta_0, \theta_{1, \epsilon},\theta_{2, \epsilon}^K,\tilde{\theta}_{3, \epsilon}^K, \theta_{4, \epsilon}^K)$. \\ For $\theta$ satisfying $ \| \theta \| > \kappa \tilde{\theta}_\epsilon$,where $\kappa > 1$ we have 
\begin{align*}
...
\hspace{-.1in} d\theta' \Omega(\theta') q(\theta' | \theta)\int dW'P(W' | W, \theta, \vartheta, \theta', X)\alpha(\theta, \theta' ; W', X)
\assign I_{1,\epsilon} + I_{1,\epsilon}^c. \\
%
...
%\left[  \int_{\E_\epsilon^c} I_1(W, \theta', \theta, \vartheta) \Omega(\theta') d\theta'  \right]^2 
  \left( I_{1,\epsilon}^c\right)^2 &\le \left[  \int_{\E_\epsilon^c}  \Omega(\theta') q(\theta' | \theta) P(W' | W, \theta, \vartheta, \theta', X)d\theta' \right]^2\\
...
$
Now for $\theta$ satisfying $ \| \theta \| >\max(\theta_2, \theta_3, \theta_\epsilon)$, we have 
\begin{align*}
  \int \Omega(\theta') P(d\theta'| W, \theta, \vartheta)
...
\end{align*}
\boqian{
We also need to specify the constant $M$ from assumption \ref{asmp:ideal_geom}.
}
