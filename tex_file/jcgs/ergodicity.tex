...
\begin{assumption}
For any positive $\epsilon > 0$, there exists $\theta_7 > 0$ and $M > 1$, such that $P(\frac{\Omega(\theta')}{\Omega(\theta)} \in [1/M, M] | \theta) > 1 - \epsilon$, for $\theta \in B_{0, \theta_7}^c$. 
\end{assumption}
%\begin{proof}
%From the assumptoin, there exists $\theta_5 > 0$ such that $p(\theta)$ is decreasing with respect to $\Vert \theta \Vert$ for $\theta \in B_{0, \theta_5}^c$. For $\theta > \max(\theta_2, \theta_5)$, $\theta' \in B_{\theta, h}$ with probability greater than $1 - \epsilon$. 
%\begin{align*}
...
it is easy to see that the eigenvalues of $B(\theta,\theta')$ satisfy 
$\lambda_B(\theta, \theta') = 
1 - \frac{\lambda_A(\theta)}{\Omega(\theta, \theta')}$. In particular,
the second eigenvalue $\lambda^2_B(\theta,\theta')$ of $B$ (which determines 
mixing properties of $B$) equals $1 - \frac{\lambda_A(\theta)}{\Omega(\theta, \theta')}$.
Assumption~\ref{asmp:cond_num} requires that $\lambda^A_2(\theta)$ satisfies 
$\lambda^A_2(\theta) \geq \mu \Omega(\theta)$. 
This, together with the assumption of the lemma, gives 
...
\begin{proposition}
Write $P(W', \theta' | W, \theta, \vartheta)$ for the transition kernel of our MCMC sampler given the observations. For any positive $\epsilon$, there exist $\theta_1 > 0$ and corresponding $W_1 > 0$, such that for any $\theta \ge \theta_1$, and any $\vartheta$, we have
  $P(|W'| > W_1, \theta' > \theta_1  | W, \theta, \vartheta) \ge 1-\epsilon$.  $| \alpha_I(\theta,\vartheta,X) - \alpha(\theta,\vartheta,W',X)| 
  \le \epsilon $ for any $(W', \theta')$ with $|W'| > W_1$ and $\theta' > \theta_1$.
  %$(\theta,\theta^*)$ satisfying
\label{prop:mix0}
\end{proposition}
\begin{proof}
From previous lemma, for any $\epsilon > 0$ and a determined $K_0 > 1$, given $\frac{1}{K_0}  \leq \frac{\Omega(\theta')}{\Omega(\theta)}  \leq K_0 $, $\exists \theta_0 > 0, W_0 > 0$, such that for any $|W| > W_0$ and $\theta > \theta_0$ and $\theta' > \theta_0$, we have $|\frac{P(X | W, \theta' , \theta)}{P(X | W, \theta , \theta')} - \frac{P(X | \theta')}{P(X | \theta)}| < \epsilon / 2$. 
\begin{align*}
