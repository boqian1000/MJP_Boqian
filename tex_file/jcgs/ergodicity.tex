...
independently under the stationary distribution. Write $|\Delta W_X|$ for the 
minimum number of grid points between two successive observations. So 
long as $|\Delta W_X| > w_0$ for large enough $w_0$,  
...
Then for any $W$ with $| \Delta W_X| > w_0$, we have
\begin{align*}
|P(X|W , \theta, \theta') - P_{st}(X | \theta)| &= | \sum_{v} P(X | v, \theta)(P^{|W|}(v | W, \theta, \theta') -  P_{st}(v | \theta))|\\
& \leq |\sum_{v} P(X | v, \theta)|P^{|W|}(v | W, \theta, \theta') -  P_{st}(v | \theta)|\\
& \leq \sum_{v} P(X | v, \theta){\epsilon'}\\
& \leq N\xi\epsilon' := \epsilon''
\end{align*}
%So for $W$ with $|W| > W_0$, we have  $|P(X| W, \theta, \theta') - P_{st}(X | \theta)| < \epsilon / 4$. \\
Using the same $w_0$ as before, we can get a similar result for the 
continuous case.
\begin{align*}
P(X | \theta) &= \int_W dW P(X , W | \theta, \theta')\\
&= \int_{|W| > w_0} + \int_{|W| \leq w_0}dW P(X | W, \theta, \theta') P(W | \theta, \theta') 
...
% \end{align*}
On the set $\{|W| > w_0\}$, $|P(X |W, \theta, \theta')-P_{st}(X|\theta)| 
\le \epsilon''$, and and by choosing $\theta'$ large enough, this occurs 
...
distribution of our MCMC sampler. For any $\epsilon$, there exist 
$\theta_\epsilon$ and $W_\epsilon > 0$, such that for 
...
$\epsilon$, we can find a $\theta_2$ such that for all $\theta > \theta_2$, 
$$P(E^K_2 | \theta) > 1 - \epsilon .$$

On this set $E^K_2$, $\Omega(\theta') \le K \Omega(\theta)$ (and also 
$\Omega(\theta) \le K \Omega(\theta')$), and 
lemma~\ref{lem:eigenvalue_lemma} ensures that 
%for $\epsilon$ and $K_3$ and $K_2$, given 
%$\frac{1}{K_3}  \leq \frac{\Omega(\theta')}{\Omega(\theta)}  \leq K_3 $, 
there exist $\theta_3 > 0, W_3 > 0$, such that for any $|W| > W_3$ and 
$\theta > \theta_3$ and $\theta' > \theta_3$, we have 
$|P(X | W, \theta' , \theta) - P(X | \theta' , \theta)| < \epsilon$, and
$|P(X | W, \theta , \theta') - P(X | \theta , \theta')| < \epsilon$. 
Together with the fact that the likelihoods are lower-bounded by $\eta$,
this implies that
\begin{align*}
|\frac{P(X | W, \theta' , \theta)}{P(X | W, \theta , \theta')} - \frac{P(X | \theta')}{P(X | \theta)}| < \epsilon / \max(K_2, 1).
\end{align*}
Call the event $E_3$. The probability of $E_3$ given $E_2$ is the 
probability $|W| > W_3$ given $E_2$. As $\theta$ increases, this 
probability can be brought arbitrarily close to $1$.
...
Given current state $(W, \theta)$, set $\theta_\epsilon = \max(\theta_2,\theta_3, \theta_4, \theta_5, \theta_6)$.  
...
& \ge P(E_1) + P(E_2) - 1\ge 1 - \epsilon / 2
\end{align*}
