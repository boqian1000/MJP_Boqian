...
$(\theta,\vartheta')$, proposing to swap $(\theta,\vartheta')$ and then 
...
P(V_{i + 1} = s^* | V_i = s^*,X,W,\theta,\vartheta) \\ %\prod P(X_{[w_i, w_{i + 1})} | v_i = s_0, \theta)\\
...
\quad \text{(since by assumption, $\Omega(\theta) \le M$)}.
...
\label{eq:acc}
\end{align*}
Using assumption \ref{asmp:ideal_geom} to bound $q(\theta'|\theta)\alpha_I(\theta',\theta;X)$,
and since by assumption, $|W| \le h$, %from equations~\eqref{eq:marg} and~\eqref{eq:acc}, 
\begin{align*}
