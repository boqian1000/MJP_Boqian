...
\begin{lemma}
Given an observation observed at $t_{end}$, and $h > 0$, $\theta$ and $\theta'$, with $\parallel \theta'  - \theta \parallel \leq h$, for $\forall \epsilon$, $\exists  W_0, B_\theta > 0$ such that for all $W$ with $|W| > W_0$, and $\theta > B_\theta$ we have $|P(X| W, \theta, \theta') - P(X | \theta)| < \epsilon$
\label{lem:eigenvalue_lemma}
\end{lemma}
\begin{proof}
From our assumption 1, there exists $\mu'$ such that $\lambda^A_2(\theta) \geq \mu \Omega(\theta, \theta')$. Since $B(\theta, \theta') = \frac{A(\theta)}{\Omega} + I $, there is a one to one mapping between the eigen values of B and the eigen values of A. $\lambda_B(\theta, \theta') = 1 - \frac{\lambda_A(\theta)}{\Omega(\theta, \theta')}$. So the second largest eigen value of the transition probability matrix $B(\theta, \theta')$ is less or equal to a constant which is less than 1. It leads to the following. for any $\epsilon > 0$ there exists $W_0'$ for $W$ with $|W| > W_0'$ we have $\parallel P^{(|W|)}(\cdot | W, \theta, \theta') - p_{st}(\cdot | W, \theta, \theta')\parallel_{TV} \leq \epsilon$. Where $P^{(t)}(\cdot | W, \theta, \theta' )$
 is the t-step transition probability with respect to the Markov chain with transition matrix $B(\theta, \theta')$ and the determined time grid $W$, while $p_{st}$ is the stationary distribution.
\begin{align*}
P(X, W | \theta, \theta') &= \sum_V P(X | V, W, \theta, \theta') P(V | W, \theta, \theta') P(W | \theta, \theta')\\
&= \sum_{V_{|W|}} P(X | V_{|W|}, \theta, \theta') P^{(|W|)}(V_{|W|} | W, \theta, \theta') P(W|\theta, \theta')\\
\end{align*}
For $W$ with $|W| > W_0$, we have  $|P(X| W, \theta, \theta') - P_{st}(X | \theta)| < \epsilon$. \\
Similarly, we have
\begin{align*}
P(X | \theta) &= P(X| \theta, \theta')= \sum_W P(X, W | \theta, \theta')\\
&= \sum_W \sum_{V_{|W|}} P(X | V_{|W|}, \theta, \theta') P^{(|W|)}(V_{|W|} | W, \theta, \theta') P(W|\theta, \theta')\\
&= \sum_{W s.t. |W| \leq W_0'} \sum_{V_{|W|}} P(X | V_{|W|}, \theta, \theta') P^{(|W|)}(V_{|W|} | W, \theta, \theta') P(W|\theta, \theta') + \\
&\sum_{W s.t. |W| > W_0'} \sum_{V_{|W|}} P(X | V_{|W|}, \theta, \theta') P^{(|W|)}(V_{|W|} | W, \theta, \theta') P(W|\theta, \theta')
\end{align*}
For $ \epsilon$, there exists $B_\theta > 0$, such that for $\parallel \theta \parallel  > B_\theta$, $|P(X | \theta) - P_{st}(X | \theta)| < \epsilon$.
\end{proof}

\begin{proposition}
Write $P(W', \theta' | W, \theta, \vartheta)$ for the transition kernel of our MCMC sampler given the observations. For any positive $\epsilon$, there exist $B_\theta > 0$ such that for all $\theta \ge B_\theta$, we have
  $P(\{(W', \theta') \ s.t.\ |\alpha_I(\theta,\vartheta,X) - \alpha(\theta,\vartheta,W',X)| 
  \le \epsilon\} | W, \theta, \vartheta) \ge 1-\epsilon$.%$(\theta,\theta^*)$ satisfying
\label{prop:mix0}
\end{proposition}
\begin{proof}
From previous lemma, for any $\epsilon > 0$, $\exists B_\theta > 0, W_0 > 0$, if $\parallel \theta - \theta' \parallel < h$ and $|W| > W_0$ and $\theta > B_\theta$, then  $|P(X | W, \theta, \theta') - P(X | \theta)| < \epsilon$. 
\begin{align*}
