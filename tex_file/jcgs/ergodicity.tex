...
%For $\epsilon/4 > 0$, 
From assumption \ref{asmp:omega}, given any $\epsilon,K$, we can find 
$\theta_3 > 0$ such that for $\theta > \theta_3$, 
$$P(\frac{\Omega(\theta')}{\Omega(\theta)} \in [{1}/{K_3},  K_3] | \theta) 
> 1 - \epsilon .$$

On this set, lemma \ref{lem:eigenvalue_lemma} ensures that 
%for $\epsilon$ and $K_3$ and $K_2$, given 
%$\frac{1}{K_3}  \leq \frac{\Omega(\theta')}{\Omega(\theta)}  \leq K_3 $, 
there exist $\theta_4 > 0, W_4 > 0$, such that for any $|W| > W_4$ and $\theta > \theta_4$ and $\theta' > \theta_4$, we have \begin{align*}
