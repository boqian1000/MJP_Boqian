...
observations, with the distribution over states brought arbitrarily 
...
independently under the stationary distribution. For $\mW > w_{0}$ for 
large enough $w_0$,
...
& \leq \sum_{V_X} P(X | V_X, \theta){\epsilon'} \le \epsilon''
%& \leq N \| X \| \ub \epsilon' := \epsilon''
\end{align*}
%So for $W$ with $|W| > W_0$, we have  $|P(X| W, \theta, \theta') - P_{st}(X | \theta)| < \epsilon / 4$. \\
\vinayak{$p(X|\theta)$ is ambigious}
For $\theta$ large enough, we get a similar result in the continuous 
case. First, 
for any $\theta'$, with uniformization rate $\Omega(\theta,\theta')$, 
%\begin{align*}
$
