...
%\end{definition}
%}
\boqian{$\inf \pi_\theta$ assumption?}
%\begin{assumption}
...
Recall $B(\theta, \theta') = I+\frac{A(\theta)}{\Omega(\theta, \theta')}$
...
counterpart embedded in $W$ to be brought arbitrarily close to $\pi_\theta$ \boqian{the stationary distribution?} 
...
%\end{proof}
\begin{proposition}
The a posteriori probability \boqian{that} the embedded Markov chain makes a
self-transition,
...
P(W'| S, T, \theta', \theta)
\end{align*}
\boqian{Suppose $(W, \theta, \vartheta) \in B_{h, M}$.}
...
\end{align*}
\boqian{where $F_{Poiss(\Omega(\vartheta''))}$ is the Poisson CDF with rate $\Omega(\vartheta'')$.}
We have
\begin{align*}
...
than $\Omega(\theta')$ by a sufficient factor. \boqian{also has a lower bound}
...
Our proof strategy is to show that for $\theta$ and $W$ large enough \boqian{for $\| \theta \|$ and $\mW$ large enough}, the
...
&\le   \int d\nu dW'  q(\nu | \theta)
  ...
  [ 1 - (\alpha_I(\theta, \nu ; X)-\epsilon)] \\
  &\le (1+\epsilon)  - \int  q(\nu | \theta) \alpha_I(\theta, \nu ; X) d\nu, \\
 & \text{while bounding the acceptance \boqian{rejection} rate by 1,} \\
...
& \le \left[  \int_{\E_\epsilon^c}  \Omega(\theta') q(\theta' | \theta) P(W' | W, \theta, \vartheta, \theta', X)d\theta' \right]^2\\
