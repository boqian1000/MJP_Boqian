...
$\int \Omega(\nu) p_I(\nu|\theta) d\nu
\leq (1-\rho) \Omega(\theta)+L$, $\forall \theta \not\in \SM_M$.
...
\end{corollary}
\begin{proof}
From assumption \ref{asmp:integ_bound},  we have $ \int_\Theta \Omega(\mu)^2 q(\nu | \theta)d\nu \leq \eta_0 \Omega(\theta)^2$ for $\theta$ satisfying $\| \theta \|  > \theta_2$.
...
\begin{assumption}
For any $\epsilon>0$,  there exist finite $M_\epsilon$, $\theta_{4,\epsilon}$
...
\end{align*}
By corollary \ref{corol:integ_bound}, there exist $\eta_1 , \theta_3$ such that for 
%$\theta$  satisfying 
$ \| \theta \| > \theta_3$, 
 $ \int \Omega(\nu) q(\nu | \theta)d\nu \leq \eta_1 \Omega(\theta) 
 $. Then,
...
$.\\
Again, the first term involves an integral over $(\theta',W')$ which 
we divide into two regions $\E_\epsilon$ and its complement
$\E^c_\epsilon$:
...
& \assign (1 - \delta_1)\lambda_1 |W| + (1 - \delta_2)\Omega(\theta)
\end{align*}
...
%\end{align*}
      Then the drift condition holds with $\delta = \min(\delta_1,\delta_2)$.
