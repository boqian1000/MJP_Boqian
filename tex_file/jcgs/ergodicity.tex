...
Poisson events must be large (to allow sufficient transitions), and 2) the 
transition matrix $B$ must to mix arbitrarily poorly (otherwise even a 
...
and interval $[\theta-h,\theta+h]$ such that $q(\theta^* \in [\theta-h,\theta+h]|\theta)
\ge 1-\epsilon$. Thus for any $\epsilon_1$, by choosing $\theta$ large 
enough, we can ensure that %$\Omega(\theta) \approx \Omega(\theta^*)$ (i.e.\ 
$\frac{\Omega(\theta)}{ \Omega(\theta^*)} \in [1-\epsilon_1,1+\epsilon_1]$) 
with probability greater than $1-\epsilon_1$.
...
I_{2,S_{\epsilon}} \le \int_{S_\epsilon}dW' d\nu q(\nu | \theta) &
  \sum_{S,T} P(S, T | W, \theta, \vartheta, X)P(W' | S, T, \theta, \nu) 
  ( 1 - [\alpha_I(\theta, \nu | X)-\epsilon]) \\
= \int_{S_\epsilon}dW' d\nu q(\nu | \theta) &
  P(W' | S, T, \theta, \nu) 
  ( 1 - [\alpha_I(\theta, \nu | X)-\epsilon]) \\
  \le (1-\epsilon)^2 & - \int_{S_\epsilon} d\nu q(\nu | \theta) \alpha_I(\theta, \nu | X) 
%   \intertext{Then}
...
divide into two regions $I_{1,S_\epsilon}$ and its complement 
$I_{1,S^c_\epsilon}$. For the latter, we bound the acceptance probability 
by one to get
\begin{align*}
  I_{1,S^c_\epsilon} &\le  \int_{S^c_\epsilon} d\theta'dW' \Omega(\theta')
\end{align*}
