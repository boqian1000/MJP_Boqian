...
\end{align*}
For the second one, from assumption \ref{asmp:integ_bound}, there exists $\eta_0 , \theta_0 > 0$ such that for $\theta \in B_{0, \theta_0}^c$, we have 
\begin{align*}
\int_\Theta \Omega(\nu)^2 q(\nu | \theta)d\nu \leq \eta_0 \Omega(\theta)^2.
\end{align*}
So, from Cauchy inequality, we have 
\begin{align*}
