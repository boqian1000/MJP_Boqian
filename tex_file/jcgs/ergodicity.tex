...
\begin{lemma}
For any positive $K > 0$ and $\epsilon > 0$, there exists $\theta_6 > 0$ such that $P(\frac{p(\theta')}{p(\theta)} \le K | \theta) > 1 - \epsilon$, for $\theta \in B_{0, \theta_6}^c$. 
\end{lemma}
\begin{proof}
...
\begin{align*}
\log(\frac{(\theta')}{p(\theta)}) &\le \log(\frac{p(\theta - h)}{p(\theta)}) = \log(f(\theta - h)) - \log(f(\theta))\\ 
