...
$\prod P(x_o | s_o, \theta) \in [\eta_1, \xi_1]$.\vinayak{Is this for all $s_o$?}
  \label{asmp:obs_bnd}
\end{assumption}
...
\begin{assumption}
Set $\Omega(\theta) \doteq k \max_s |A_{ss}(\theta)|$, for $k > 1$ in the sampler. Given the proposal density $q(\nu | \theta)$, $\exists \eta_0 > 0, \theta_0 > 0$ such that for $\theta \in B_{0, \theta_0}^c$, $$ \int_\Theta \Omega(\mu)^2 q(\nu | \theta)d\nu \leq \eta_0 \Omega(\theta)^2.$$ 
...
\begin{assumption}
For any positive $\epsilon > 0$, there exists $\theta_2$ and $K$ 
such that $P(\frac{p(\theta')q(\theta|\theta')}{p(\theta)q(\theta'|\theta)} \le K | \theta) > 1 - \epsilon$, 
for $\theta \in B_{0, \theta_2}^c$. 
  \label{asmp:prior}
\end{assumption}
%\begin{proof}
...
\begin{assumption}
For any positive $\epsilon > 0$, there exists $\theta_3 > 0$ and $M > 1$, such that $P(\frac{\Omega(\theta')}{\Omega(\theta)} \in [1/M, M] | \theta) > 1 - \epsilon$, for $\theta \in B_{0, \theta_3}^c$. 
  \label{asmp:omega}
\end{assumption}
%\begin{proof}
%From the assumptoin, there exists $\theta_5 > 0$ such that $p(\theta)$ is decreasing with respect to $\Vert \theta \Vert$ for $\theta \in B_{0, \theta_5}^c$. For $\theta > \max(\theta_2, \theta_5)$, $\theta' \in B_{\theta, h}$ with probability greater than $1 - \epsilon$. 
...
%- P_{st}(\cdot | \theta)\parallel_{\text{TV}} \leq 
%{\epsilon'}$. 
...
for the state of the Markov chain at these times. 
...
independently under the stationary distribution. Write $\mW$ for the 
minimum number of grid points between two successive observations. So 
...
& \leq |\sum_{v} P(X | V_X, \theta)|P^{|W|}(V_X | W, \theta, \theta') -  P_{st}(V_X | \theta)|\\
& \leq \sum_{V_X} P(X | V_X, \theta){\epsilon'}\\
& \leq N\xi\epsilon' := \epsilon''
\end{align*}
...
&= \int_{\mW > w_0} + \int_{\mW \leq w_0}dW P(X | W, \theta, \theta') P(W | \theta, \theta') 
...
with arbitrarily high probability. Since the likelihood is bounded, the 
...
Then, for any $\epsilon$, there exist $\theta_\epsilon$ such that
for $\theta \ge \theta_\epsilon$, we have
...
$\theta_1$  such that {for all } $\theta \ge \theta_1$,
\begin{align*}
...
$\epsilon$, we can find a $\theta^K_2$ such that for all $\theta > \
\theta^K_2$, 
...
$\theta > \theta^K_3$ and $\theta' > \theta^K_3$, we have 
...
such that for $\theta > \theta_4$, this probability can brought 
...
{Consider the second term $I_2$, involving an integral over $(\nu,W')$. 
  ...
  exists $\theta_0 > 0$ and $W_0 > 0$ such that the set $E_{\epsilon}$ 
  (where $|\alpha(\theta, \nu | X,W') - \alpha_I(\theta, \nu | X)| \le 
...
%\end{align*}
$$\int d\theta' \Omega(\theta') I_1 = \tilde{I}_{1,\E_\epsilon} + \tilde{I}_{1,\E^c_\epsilon}.$$
For $\tilde{I}_{1,\E_\epsilon}$, we bound the acceptance probability by $\alpha_I + \epsilon$ to get 
\begin{align*}
\tilde{I}_{1,\E_\epsilon} &= \int_{\E_\epsilon} I_1 \Omega(\theta') d\theta'\\
...
\end{align*}
For $\tilde{I}_{1,\E^c_\epsilon}$, from assumption \ref{asmp:integ_bound}, there exists $\eta_1 , \theta_3 > 0$ such that for $\theta \in B_{0, \theta_3}^c$, we have 
\begin{align*}
\int_\Theta \Omega(\nu)^2 q(\nu | \theta)d\nu \leq \eta_1 \Omega(\theta)^2.
\end{align*}
So, by Cauchy inequality and bounding the acceptance probability by one, we have 
\begin{align*}
\tilde{I}_{1,\E_\epsilon^c}^2 &=  \left[  \int_{\E_\epsilon^c} I_1 \Omega(\theta') d\theta' \right]^2 \\
...
& \le  \eta_1 \Omega(\theta)^2 \epsilon.
\end{align*}
So we have 
\begin{align*}
\tilde{I}_{1,\E_\epsilon^c} \le \sqrt{\eta_1}\Omega(\theta) \sqrt{\epsilon}.
\end{align*}
Then for $\theta > \max(\theta_0, \theta_1, \theta_2, \theta_3)\doteq \theta_\epsilon$, we have the following. 
\begin{align*}
  \int \Omega(\theta') P(d\theta'| W, \theta, \vartheta)  
  ...
  &\sqrt{\eta_1}\Omega(\theta) \sqrt{\epsilon} +  \eta_0 \epsilon \Omega(\theta) + \epsilon \Omega(\theta)\\
  & \leq (1 - \rho) \Omega(\theta) + \sqrt{\eta_1}\Omega(\theta) \sqrt{\epsilon} +  \eta_0 \epsilon \Omega(\theta) + \epsilon \Omega(\theta)
\end{align*}
For $\theta > \theta_\epsilon$, we have \boqian{need to change afterwards}
\begin{align*}
