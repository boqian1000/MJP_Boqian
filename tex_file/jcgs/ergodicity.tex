...
\label{def:mjp_symm}
\end{definition}
This definition is from~\cite{fill1991}, who show \boqian{shows} that the matrix 
...
\ge \frac{\mu}{K_0+1} $. 
\boqian{not consistent with the assumption 3, we need to assume $ \frac{\lambda^{R_A}_2(\theta)}{\Omega} \ge \mu$}
Also, 
\begin{align*}
...
probability measure $\phi_1$ independent of the initial state \boqian{in $B_{h, M}$}.
\label{lem:small_set}
\end{lemma}
...
\end{align*}
For $\int_{\E_\epsilon^c} I_1(W, \theta', \theta, \vartheta) \Omega(\theta') d\theta'$, from assumption \ref{asmp:integ_bound}, there exists $\eta_0 , \theta_2 > 0$  \boqian{we can delete $\theta_2$ here} such that for $\theta$ satisfying $ \| \theta \| > \theta_2$, we have
$\int_\Theta \Omega(\nu)^2 q(\nu | \theta)d\nu \leq \eta_0 \Omega(\theta)^2$.
