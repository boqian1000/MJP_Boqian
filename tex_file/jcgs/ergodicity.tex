...
%$\eta_0 <1$. }
\begin{assumption}
$\exists \rho_1 \geq \left(1 - \frac{\eta_1^2}{\xi_1^2}\right)\sup_\theta \int q(\mu| \theta)\alpha_I(\theta, \mu)d\mu $ and $\rho_1 \leq \sup_\theta \int q(\mu| \theta)\alpha_I(\theta, \mu)d\mu$, s.t.
$$\int \max_s A_s(\nu)q(\nu | \theta)d\nu + (1 - \int q(\mu | \theta)\alpha_I(\theta, \mu)d\mu)\max_s A_s(\theta) \leq (1 - \rho_1) \max_s A_s(\theta), \forall \theta \in \Theta.$$
...
\begin{align*}
\int_\Theta \max_sA_s(\nu)P(d\nu| W, \theta, \theta^*)  &\leq \int \max_sA_s(\nu) q(\nu | \theta) d\nu + \left(1 - \frac{\eta_1^2}{\xi_1^2} \int_C q(\mu | \theta)\alpha_I(\theta, \mu) d\mu\right) \max_sA_s(\theta) \\ 
