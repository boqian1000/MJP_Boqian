...
%MJPs model temporal evolution in continuous-time, 
resulting in 
...
$\sum_{j \neq i} A_{ij} = -A_{ii} := A_i$, \boqian{which one should we use as definition $\doteq$ or $:=$?} so that the rows of $A$ sum to $0$.  
$A_i$ gives the total rate at which the system leaves state $i$ for any other state.
To simulate an MJP over an interval $[0,\cT]$, one follows 
...
\end{itemize}
The times $T=(t_0, \cdots, t_{|T|\boqian{|T| - 1} })$ and states 
$S=(s_0, \cdots, s_{|T|\boqian{|T| - 1} })$ define the MJP path $S(t)$.
