...
s1 = 5
s2 =2
p_ALPHA <- ggplot() + theme_bw()+ theme(axis.text=element_text(size=40), axis.title=element_text(size=40)) + coord_fixed(ratio = ratio.values)+ ylim(0, maxy)+ xlim(0, maxx)+
...
ggsave(filename, width = 9, height = 9, device = pdf)
...
p_BETA <-ggplot() + theme_bw()+ theme(axis.text=element_text(size=40), axis.title=element_text(size=40)) + coord_fixed(ratio = ratio.values) + ylim(0, maxy) + xlim(0, maxx)
p_BETA <- p_BETA + 
