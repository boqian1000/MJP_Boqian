setwd( "/Users/Isaac_Zhang/Research/MCMC/simulation_result/data/q_3/")
library(ggplot2)
load("rda_var")
load("gbs_a")
load("gbs_b")
...
ggsave("mh_q_alpha_dim3.pdf", height = 6.8, width = 6.8)
...
p_BETA <- p_BETA +   ylim(0,maxy) + xlim(0, maxx)+scale_shape(solid = FALSE) + 
  geom_point(data = mh_b, aes(x = var, y =mh_b_k1.5) , colour = "skyblue3", size = s1,alpha = 0.8, shape = 1, stroke = 2) +
  geom_errorbar(data = mh_b, aes(x= var,ymax = mh_b_k1.5u, ymin = mh_b_k1.5d), width=0.05, alpha = 0.8,colour = "skyblue3")+
  geom_line(data = mh_b, aes(x = var, y =mh_b_k1.5) ,colour = "skyblue3", size = s2, alpha = trans_col) +
  geom_point(data = mh_b, aes(x = var2, y =mh_b_k2) , colour = "skyblue3", size = s1, alpha = 0.8,shape = 0, stroke = 2) +
  geom_errorbar(data = mh_b, aes(x= var2, ymax = mh_b_k2u, ymin = mh_b_k2d), width=0.05,alpha = 0.8, colour = "skyblue3")+
  geom_line(data = mh_b, aes(x = var2, y =mh_b_k2) ,colour = "skyblue3", size = s2, alpha = trans_col) +
  geom_point(data = mh_b, aes(x = var3, y =mh_b_k3) , colour = "skyblue3", size = s1,alpha = 0.8, shape = 2, stroke = 2) +
  geom_errorbar(data = mh_b, aes(x= var3,ymax = mh_b_k3u, ymin = mh_b_k3d), width=0.05,alpha = 0.8, colour = "skyblue3")+
...
ggsave("mh_q_beta_dim3.pdf", height = 6.8, width = 6.8)
