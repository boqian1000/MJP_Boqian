...
on both the current and proposed parameter, we consider two settings
 ...
 ($\kappa = 1$ and $1.5$, plotted with {triangles} \boqian{circles} and {squares} \boqian{triangles}), and 
$\Omega(\theta, \theta^*) = \kappa \max(\max A(\theta), \max A(\theta^*))$
($\kappa=1.5$, plotted with {circles} \boqian{squares}).  We compare these
...
we used three settings, $\kappa=1.5,2,3$, plotted with circles, {triangles} \boqian{squares} and
{squares} \boqian{triangles}.  Unless specified, our results were
...
Among the three setting of our algorithm, the simple additive setting
 ({triangles} \boqian{circles}) does best, though it is not significantly better than the {max-of-max}\boqian{additive}
 setting \boqian{with a multiplicative factor of $2$}(triangles). \boqian{It suggests that the additive setting is more stable than the max-of-max setting when the proposal variance is large.}The {additive setting with a multiplicative factor of $1.5$} \boqian{max-of-max setting}
 (squares) does worse than both {additive choice with smaller multiplicative factor and the max-of-max choice}\boqian{additive choices} but still better than the other algorithms. Among the baselines, simple Gibbs sampling 
