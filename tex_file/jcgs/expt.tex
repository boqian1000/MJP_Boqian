...
In the following, we evaluate Python implementations of a number of algorithms, including the \naive\ MH algorithm (Algorithm~\ref{alg:MH_naive}, which we plot \boqian{as yellow twodashed  lines}), and our main contribution, the symmetrized MH algorithm (algorithm~\ref{alg:MH_improved}, plotted \sout{as dashed red lines}\boqian{as solid blue lines}
\vinayak{Do this}). 
...
Figure~\ref{fig:TRACE_EXP} shows the MCMC estimates of the posterior distribution over $\alpha, P(\alpha|X)$ from the Gibbs sampler as well as our symmetrized MH sampler. Visually these agree, and we quantify this by running a two sample-Kolmogorov Smirnov test: the result is a p-value of $0.5085$. The figure also shows traceplots and autocorrelation plots for $\alpha$ from the two samplers:
 clearly our sampler mixes much more efficiently than Gibbs.
  ...
  \begin{minipage}[!hp]{0.74\linewidth}
    \caption{(Left) posterior over $\alpha$ produced by Gibbs \sout{(black)} \boqian{red-dashed} and symmetrized MH \sout{red-dashed} \boqian{blue solid}). (Below) trace plot and autocorrelation plots for Gibbs (left) and symmetrized MH (right). All plots are for dimension $10$. \ \ }
     \label{fig:TRACE_EXP}
  \end{minipage}
  \centering
...
Among the three setting of our algorithm, the simple additive setting
 (\sout{triangles} \boqian{square}) does best, though it is only slightly better than 
...
%  \begin{minipage}[!hp]{0.3\linewidth}
    \caption{ESS/sec for the synthetic  model, the top row being dimension 3, and the bottom, dimension 10. The left and right columns are for $\alpha$ and $\beta$. \sout{Red, yellow, blue and black} \boqian{Blue solid, yellow dashed, red long-dashed and black two-dashed} curves are the symmetrized MH, \naive\ MH, Gibbs and particle MCMC algorithm. Different symbols correspond to different settings of the algorithms, see section~\ref{sec:expts}}
...
\end{minipage}
  ...
  \centering
	...
	\end{minipage}
    \caption{ESS for the synthetic model, the top row being dimension 3, and the bottom, dimension 10. The left and right columns are for $\alpha$ and $\beta$. \sout{Blue, yellow and red}\boqian{Blue solid, yellow dashed, red long-dashed} curves are the symmetrized MH, \naive\ MH, and Gibbs algorithm. Different symbols correspond to different settings of the algorithms, see section~\ref{sec:expts}}
...
%    \caption{Time Interval vs. ESS / sec}
  \caption{Time interval vs ESS/s for the synthetic MJP. In the left two plots, the number of observations is fixed, in the right two, this grows linearly with the interval length. \sout{Squares, triangles and circles} \boqian{Blue solid, yellow dashed and red long-dashed curves} are the symmetrized MH, \naive\ MH and Gibbs algorithm.
...
%  \begin{minipage}[!hp]{0.28\linewidth}
  \caption{(a) Jukes-Cantor (JC69) model, (b)
    ESS/sec for the JC69 Model. sout{Red, yellow and blue} \boqian{Blue solid, yellow dashed and red long-dashed curves} curves are the 
      symmetrized MH, \naive\ MH and Gibbs algorithm.(c)
    ESS for the symmetrized MH for the JC69 Model.(d)
    ESS for the JC69 Model. \sout{Red, yellow and blue} \boqian{Blue solid, yellow dashed and red long-dashed curves} curves are the 
      symmetrized MH, \naive\ MH and Gibbs algorithm. }
     \label{fig:ESS_JC}
 % \end{minipage}
 \end{figure}
...
%  \end{figure}
  ...
  \end{minipage}
  \begin{minipage}[!hp]{0.4\linewidth}
    \caption{Time Interval vs.\ ESS/sec for JC model. Number of observations is fixed (left), and grows linearly with interval (right). \sout{Squares, circles and triangles}\boqian{Blue solid, yellow dashed and red long-dashed curves} are the symmetrized MH, \naive\ MH and Gibbs. }
	\label{fig:jc_model_vs_t}
  \end{minipage}
%  \vspace{-.4in}
  \end{figure}
...
\end{minipage}
  ...
  \centering
	...
	\end{minipage}
%  \begin{minipage}[!hp]{0.3\linewidth}
    \caption{ESS/sec for the immigration model, the left two being dimension 3, and the right two, dimension 10. The left and right columns are for $\alpha$ and $\beta$. \sout{Red, yellow, blue curves} \boqian{Blue solid, yellow dashed and red long-dashed curves} are the symmetrized MH, \naive\ MH, and Gibbs algorithm. Different symbols correspond to different settings of the algorithms, see section~\ref{sec:expts}}
     \label{fig:ESS_Q_D10}
%  \end{minipage}
  \end{figure}
...
\end{minipage}
  ...
  \centering
	...
	\end{minipage}
    \caption{ESS for the immigration model, the top row being dimension 3, and the bottom, dimension 10. The left and right columns are for $\alpha$ and $\beta$. \boqian{Blue solid, yellow dashed and red long-dashed curves} are the symmetrized MH, \naive\ MH, and Gibbs algorithm. Different symbols correspond to different settings of the algorithms, see section~\ref{sec:expts}}
...
\end{minipage}
  ...
  \centering
	...
	\end{minipage}
%  \begin{minipage}[!hp]{0.3\linewidth}
    \caption{ESS/sec for the time-inhomogeneous immigration model, the left two being dimension 3, and the right two, dimension 10. The left and right columns are for $\alpha$ and $\beta$. \sout{Blue, yellow, red curves} \boqian{Blue solid, yellow dashed and red long-dashed curves} are the symmetrized MH, \naive\ MH, and Gibbs algorithm. Different symbols correspond to different settings of the algorithms, see section~\ref{sec:expts}}
     \label{fig:ESS_pc_10}
%  \end{minipage}
  \end{figure}
...
\end{minipage}
  ...
  \centering
	...
	\end{minipage}
    \caption{ESS for the time-inhomogeneous immigration model, the top row being dimension 3, and the bottom, dimension 10. The left and right columns are for $\alpha$ and $\beta$.\sout{ Blue, yellow and red curves} \boqian{Blue solid, yellow dashed and red long-dashed curves} are the symmetrized MH, \naive\ MH, and Gibbs algorithm. Different symbols correspond to different settings of the algorithms, see section~\ref{sec:expts}}
...
% not included it).  
 ...
 shifting the optimal setting of the proposal variance to smaller values.
  ...
  Our MH proposal distribution is then $q(\nu|\theta) = N(\nu|\theta, \kappa\Sigma_\theta)$ for different settings of $\kappa$ (the typical choice is $\kappa = 1$)\boqian{(we might want to change $\kappa$ here, because we use $\kappa$ in the beginning of the section for different meaning.)} \boqian{, where we set $\Omega(\theta, \vartheta) = \max A(\theta) + \max A(\vartheta)$}. 
