...
symmetrized improvement (algorithm~\ref{alg:MH_improved}, which we call
symmetrized MH). We compare different variants of these algorithms, 
corresponding to different uniformizing Poisson rates (i.e.\ different choices 
of $\kappa$, see section~\ref{sec:comments}). For \naive\ MH, %~\ref{alg:MH_naive}, 
we set $\Omega(\theta) = \kappa \max_s A_s(\theta) $ with $\kappa$  equal to 
$1.5, 2$ and $3$. For symmetrized MH, %algorithm~\ref{alg:MH_improved}, 
...
Among the three setting of our algorithm, the simple additive setting
 ...
 setting with a multiplicative factor of $2$ (squares). The max-of-max setting
