...
%    \vspace{-.81 in}
  ...
  \centering
  \vspace{-0.5in}
    \includegraphics [width=0.89\textwidth, angle=0]{figs/ECOLI_alpha.pdf}
    \includegraphics [width=0.89\textwidth, angle=0]{figs/ECOLI_l1.pdf}
%    \vspace{-.81 in}
...
% not included it).  
 ...
 shifting the optimal setting of the proposal variance to smaller values.
  ...
  We use this setup to evaluate our proposed Metropolis-Hastings sampler 
  along with Gibbs sampling. For our MH proposal distribution, we first 
  run $500$ iterations of Gibbs sampling to estimate the covariance matrix 
  of the vector $\theta = (\alpha,\beta,\lambda_1,\lambda_2)$, call this 
  $\Sigma_\theta$. Our MH proposal distribution is then $q(\theta^*|\theta) = 
  N(\theta^*|\theta, \kappa\Sigma_\theta)$ for different settings of 
  $\kappa$ (the typical choice is $\kappa = 1$). Figure~\ref{fig:ECOLI} 
  shows the results for the parameter $\alpha$, we include $\beta$ in 
  the appendix. We see that for the typical setting of $\kappa=1$, our 
  sampler ourperforms the Gibbs sampler, though Gibbs sampling does 
  outperform our method for large or small $\kappa$. This is because now, 
  the proposal variance is too large or too small, further, the Gibbs 
