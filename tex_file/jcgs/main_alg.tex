...
\begin{algorithm}[H]
   \caption{MH for parameter inference for MJPs }
  ...
  \begin{tabular}{l l}
   ...
   \begin{algorithmic}[1]
    ...
    Define $W = T \bigcup U$ and discard all MJP state information.
...
The uniformization scheme of~\cite{RaoTeh13} works for any Poisson rate 
$\Omega > \max_i A_i$, this ensures that the conditional probability
of sampling one or more thinned events $U$ is positive for
every trajectory $S(t)$ (recall 
$U \sim \text{PoissonProc}(\Omega-A_{S(t)})$). Their empirical results suggest 
setting $\Omega = 2 \max_i A_i$.

Implicit in our new scheme is a uniformizing Poisson process with rate
$\Omega(\theta,\theta') \equiv \Omega(\theta) + \Omega(\theta')$. For our 
