...
In practical situations, an MJP is only observed at
a finite set of times, with the observations themselves
noisy. This raises two questions: 
%\begin{itemize}
1) what is the trajectory underlying the observations? and 2)
  what are the MJP parameters governing the dynamics?
%\end{itemize}
  \vspace{-.1in}
\subsection{Trajectory inference for MJPs}
This was addressed in~\cite{RaoTeh13}, and was extended to a broader class of 
jump processes %(like semi-Markov processes) 
in~\cite{RaoTeh12}. %Both center on alternate approaches to 
% Gillespie's algorithm, 
Both involve simulating an MJP by introducing auxiliary 
...
Now introduce a parameter $\Omega \ge \max_i A_i$;
...
in Gillespie's algorithm, first simulate a set $W$ of candidate
