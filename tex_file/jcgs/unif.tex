...
We assume the following structure on the observation process: for any partition $W = \{w_0 = 0, w_1, \dotsc, w_{|U|}=t_{end}\}$ of the interval $[0,t_{end}]$, there exist easy to compute functions $\ell_i$ such that the likelihood factors as:
\begin{align}
  \label{eq:lik_factor}
  P(X|S(t),\ t \in [0,t_{end}]) = \prod_{i=1}^{|G|-1} \ell_i(S(t),\ t \in [w_{i-1},w_i])
\end{align}
...
With $A(\cdot)$ and $\pi$ assumed known, the overall Bayesian model is then 
\begin{align}
...
\State \textbf{
  Discard state information $S$, and simulate new state values $V$ on the thinned and actual transition times $W = T \cup U$ 
}:

\noindent This is equivalent to simulating a path from a discrete-time hidden Markov model (HMM) on the times $W$, with initial distribution $\pi_0$ and transition matrix $B$.
Between two consecutive elements of $W$, the state remains fixed, and from equation~\eqref{eq:lik_factor}, the likelihood of state $s$ at time $w_i$ equals $\ell_i(S(t) = s,\ t \in [w_i,w_{i+1}])$ for a known $\ell_i(\cdot)$. 
Write this as $\ell_i(s)$. 
To simulate the path,~\cite{RaoTeh13} use the forward-filtering backward-sampling (FFBS) algorithm~\citep{dsfs}, which involves two steps:
\begin{description}
  \item[Forward pass:] 
  ...
  \item[Backward pass:]
    ...
    \beta_i(s) = \alpha_i(s)\cdot B_{sv_{t+1}} \cdot \ell_i(s) \quad \forall s \in S.$$
