...
from a piecewise-constant Poisson process with rate $\{\Omega(\theta)-A_{S(t)}(\theta), t \in [0,t_{end})\}$: 

$ U \sim \text{PoissProc}(\Omega(\theta) - A_{S(t)}(\theta))  \quad \text{(the rate at time $t$ is $\Omega(\theta)-A_s(\theta)$ if $S(t) = s$).}$
...
\begin{description}
  ...
  \item[Backward pass:]
    Simulate ${v}_{|{W}|} \sim \bck_{|{W}|}(\cdot)$, where $\bck_{|{W}|}(s) \propto \fwd_{|{W}|}(s)\cdot\ell_{|{W}|}(s) \quad \forall s \in \cS.$ 
