...
In practical situations, an MJP trajectory is only observed
a finite set of times, and typically, these observations themselves
are noisy. There are then two questions than the practitioner faces:
\begin{itemize}
...
that are {\em thinned} while simulating an MJP trajectory. 
We focus on the simpler, more widely used algorithm from~\cite{RaoTeh13},
which is based on an idea called {\em uniformization}~\cite{Jen1953}. 
...
Call these states $V$. Thus $v_0 \sim \pi$, while $p(v_{k+1}=j|v_k=i) = B_{ij}$
...
algorithm~\ref{alg:Unif_gibbs}. At a high-level, the algorithm proceeds by 
alternately sampling a new grid $W$ conditioned on the MJP trajectory $S(t)$, 
and then a new trajectory $S(t)$ conditioned on the grid $W$. The latter
step can be carried out using standard techniques from the discrete-time
HMM literature.
...
plots were produced from the experiment in section~\ref{sec:immig}.}
  ...
  \end{minipage}
  \end{figure}
  In any case, the Gibbs sampling approach of algorithm~\ref{alg:MJP_gibbs} 
...
as well as a realization of the MJP trajectory (in green). We have also
included the prior distribution in red. Observe how much
...
algorithm used to same a new trajectory, so that no 
