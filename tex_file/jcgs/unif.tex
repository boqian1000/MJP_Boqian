...
\end{align}
Other examples form an inhomogeneous Poisson process~\citep{FearnSher2006}, renewal process~\citep{rao2011gaussian} or even another MJP~\citep{Nodelman+al:UAI02,RaoTeh13}, modulated by $(s_0, S, T)$.
...
\begin{description}
  \item[Forward pass:] 
    ...
    Sequentially update $\fwd_i(\cdot)$ at time $\tilde{w}_i \in \tilde{W}$ given $\fwd_{i-1}$: 
        \vspace{-.1in}
        $$\textbf{for } i=1\rightarrow (|\tilde{W}|-1)\textbf{ do:} \quad \fwd_i(s') = \sum_{s \in \cS} \fwd_{i-1}(s)\cdot \ell_{i}(s) \cdot B_{ss'}(\theta), \quad \forall s' \in \cS.\qquad\qquad\quad $$
