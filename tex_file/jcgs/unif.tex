...
Again, for clarity we ignore any unknown parameters in the likelihood, otherwise we can include them in $\theta$.
We assume the observation process has the following structure: for any partition $\tilde{W} = \{\tilde{w}_0 = 0, \tilde{w}_1, \dotsc, \tilde{w}_{|\tilde{W}|}=t_{end}\}$ of the interval $[0,t_{end})$ (where $|\cdot|$ denotes the cardinality of a set), there exist known functions $\ell_i$ such that the likelihood factors as:
\begin{align}
  \label{eq:lik_factor}
...
Call these states $(v_0,V)$, where $v_0 \sim \pi_0$, while $P(v_{k+1}=j|v_k=i) = B_{ij}$ for $k \in \{0,\cdots,|W|-1\}$. 
...
Introducing the thinned variables allowed~\cite{RaoTeh13} to develop a simple and efficient MCMC sampler (algorithm~\ref{alg:Unif_gibbs}). 
...
\begin{algorithm}[H]
  ...
  \begin{tabular}{l l}
   \textbf{Input:  } %& \text{A set of partial and noisy observations $X$}, \\
                      & \text{The previous MJP path $S(t) = (s_0, S, T)$, the previous MJP parameters $\theta$}.\\ 
