...
\begin{algorithm}[H]
  ...
  \begin{tabular}{l l}
   \textbf{Input:  } & \text{A set of partial and noisy observations $X$}, \\
                      & \text{The previous MJP path $S(t) = (S, T)$, the previous MJP parameters $\theta$ \boqian{and $\pi_0$}}.\\ 
   \textbf{Output:  }& \text{A new MJP trajectory $S' (t) = (S', T')$, 
...
%conditional $p(\theta_{new}|X,S(t),\theta_{curr})$. 
  ...
  \centering
    \vspace{-0 in}
    \includegraphics [width=0.7\textwidth, angle=0]{figures_new_apr12/apr27_hist_alpha.pdf}
    \vspace{0.1 in}
  \end{minipage}
...
Markov chain}
  ...
  \begin{tabular}{l l}
   ...
   \text{previous parameters $\ctheta$ \boqian{and $\pi_0$}}.\\
...
transition matrix, and can depend \boqian{on?}in not trivial ways on the exact 
