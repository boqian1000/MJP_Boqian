...
$S_{[t_i^X, t_{i + 1}^X]} $, its values over any interval $[t_i^X, t_{i+1}^X]$ can be simulated by Gillespie's algorithm\boqian{, and denote the probability density as $q_{\theta}( S_{[t^X_{i},t^X_{i + 1}]}|S_{t^X_i} )$}. Write the conditional distribution over states at time $t^X_{i+1}$ given value at time $t^X_i$ as $f_\theta(S(t^X_{i+1})|S(t^X_i))$, so that
...
\State For $i = 1, 2, ..., n$:\\
\noindent \noindent (a) Simulate $S_{[t^X_{i -1},t^X_i]}^k|S^k_{t^X_{i-1}}$ via Gillespie's algorithm, for $k = 1,2,...,N$; \vinayak{Check below}\\
